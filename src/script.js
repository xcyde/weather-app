...
function updateRecentCities(cityCardObject) {
  ...
  if (recentCities.has(cityCardObject.id)) {
    currentMapValues = currentMapValues.filter(
      (el) => el.id !== cityCardObject.id
    );
  }
  ...
  for (
    let i = 0;
    i <
    (currentMapValues.length < RECENT_CITIES_AMOUNT
      ? currentMapValues.length
      : RECENT_CITIES_AMOUNT - 1);
    i += 1
  ) {
...
function writeRecentCitiesToLocalStorage() {
  ...
  for (let i = 0; i < recentCitiesArr.length; i += 1) {
    localStorage.setItem(
      i,
      JSON.stringify({
        id: recentCitiesArr[i].id,
        name: recentCitiesArr[i].name,
      })
    );
  }
}
...
function fillRecentCitiesFromStorage() {
  ...
  for (let i = RECENT_CITIES_AMOUNT; i > 0; i -= 1) {
...
function handleRecent(event) {
  renderCity(parseInt(event.target.id.slice(6), 10));
}
